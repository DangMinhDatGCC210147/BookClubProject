    ...
    <div class="row justify-content-between align-items-center mb-10">
        ...
        <div class="col-12 col-lg-auto mb-20">
            <div class="page-heading">
                <h3 class="title">Table <span>/ Votes</span></h3>
            </div>
                        ...
                        <tbody>
                            @foreach($posts as $post)
                                <tr>
                                    ...
                                    <td>{{ $post->idSt }}</td>
                                    <td>
                                        <img src="{{ asset($post->image) }}" alt="{{ $post->name }}" width="100">
                                    </td>
                                    <td>{{ $post->email }}</td>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <a href="{{ route('posts.edit', ['post' => $post->id]) }}" class="mr-2"><i class="ti ti-pencil text-info"></i></a>
                                            <form class="product-form delete" action="{{ route('posts.destroy', $post->id) }}" method="POST" id="deleteMemberForm{{ $post->id }}">
                                                @csrf
                                                @method('DELETE')                       
